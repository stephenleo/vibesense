...
- [ ] Task 1: Create `.github/workflows/ci.yml` — PR CI workflow (AC: 1, 2)
  - [ ] Configure workflow trigger: `on: pull_request: branches: [main]` plus `push: branches: [main]`
  - [ ] Add lint+typecheck job: `runs-on: ubuntu-latest`, steps: `npm ci` → `npm run lint` → `npm run typecheck`
  - [ ] Add build-and-package job with matrix strategy: `target: [darwin-arm64, darwin-x64, linux-x64, win32-x64]`
  - [ ] Choose correct `runs-on` per target: `darwin-*` → `macos-latest`, `linux-x64` → `ubuntu-latest`, `win32-x64` → `windows-latest`
  - [ ] Steps per matrix job: `actions/checkout@v4` → `actions/setup-node@v4` (node 20.x) → `npm ci` → `electron-rebuild -f -w node-hid` → `vsce package --target ${{ matrix.target }}` → `actions/upload-artifact@v4`
  - [ ] Upload artifact named `vibesense-${{ matrix.target }}.vsix` with correct path
  - [ ] Ensure `node-hid` and `electron-rebuild` are installed (they will be installed via Story 1.2, but note this dependency)

- [ ] Task 2: Create `.github/workflows/package.yml` — Release/pre-release workflow (AC: 3)
  - [ ] Configure workflow trigger: `on: push: tags: ['v*']`
  - [ ] Add matrix build job identical to CI but triggered by tag push
  - [ ] Add release job with `needs: build` that downloads all four `.vsix` artifacts
  - [ ] Use `softprops/action-gh-release@v2` (or `gh release create`) to upload `.vsix` files to GitHub Release
  - [ ] Detect pre-release: set `prerelease: true` when tag contains `-beta` or `-rc` (use `contains(github.ref, '-beta') || contains(github.ref, '-rc')`)
  - [ ] Ensure `GITHUB_TOKEN` permissions: `contents: write` (required for release creation)

- [ ] Task 3: Add `@vscode/vsce` as devDependency (AC: 1, 3)
  - [ ] Install `@vscode/vsce` as devDependency: `npm install --save-dev @vscode/vsce`
  - [ ] Verify `package.json` scripts include `"package": "vsce package"` (already present from Story 1.1)
  - [ ] Note: In CI, `vsce` is invoked via `npx @vscode/vsce` or installed globally; prefer `npx` to avoid local version conflicts

- [ ] Task 4: Verify `.github/workflows/.gitkeep` is replaced by real workflow files (AC: 1, 2, 3)
  - [ ] Remove `.github/workflows/.gitkeep` (or it's automatically superseded when real files are added)
  - [ ] Confirm no merge conflict with existing `claude.yml` and `claude-code-review.yml` workflows already present

- [ ] Task 5: Validate workflow YAML syntax and logic (AC: 1, 2, 3)
  - [ ] YAML lint both workflow files (no tabs, valid syntax)
  - [ ] Confirm artifact upload paths match `vsce package` output filename pattern (`vibesense-0.1.0-darwin-arm64.vsix` etc.)
  - [ ] Confirm `permissions:` block is set correctly to allow artifact upload and release creation
