 ...
 *
 * - Returns the started ControllerHAL driver on successful selection.
 * - Returns null if the device list is empty or the user dismisses the picker.
 * - Wrapped in try/catch — never throws (NFR-R1).
 */
export async function showDeviceSelector(): Promise<ControllerHAL | null> {
  try {
    ...
    // Fire confirmation haptic for DualSense only
    if (
      selected.device.vendorId === DUALSENSE_VID &&
      DUALSENSE_PIDS.includes(selected.device.productId)
    ) {
